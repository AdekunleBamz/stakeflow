;; StakeFlow Unstake Contract V3
;; Unstake NFTs and claim rewards (0.001 STX fee)
;; MAINNET VERSION V3 - References v2 staking and rewards contracts
...
(define-public (unstake-nft (token-id uint))
  (let
    (
      ;; V3: Reference the v2 staking contract (uses full address)
      (stake-info (unwrap! (contract-call? 'SP5K2RHMSBH4PAP4PGX77MCVNK1ZEED07CWX9TJT.stakeflow-staking-mainnet-v2 get-stake-info token-id) ERR-NOT-STAKED))
    ...
    ;; Claim pending rewards (V3: Reference the v2 rewards contract - uses full address)
    (try! (contract-call? 'SP5K2RHMSBH4PAP4PGX77MCVNK1ZEED07CWX9TJT.stakeflow-rewards-mainnet-v2 claim-and-distribute token-id staker))
    ;; Remove stake and return NFT (V3: Reference the v2 staking contract - uses full address)
    (try! (contract-call? 'SP5K2RHMSBH4PAP4PGX77MCVNK1ZEED07CWX9TJT.stakeflow-staking-mainnet-v2 release-nft token-id staker))
...
(define-public (emergency-unstake (token-id uint))
  (let
    ...
    ;; Collect unstake fee
    (try! (stx-transfer? (var-get unstake-fee) tx-sender TREASURY))
    ;; Remove stake and return NFT (skip rewards) (V3: Reference the v2 staking contract - uses full address)
    (try! (contract-call? 'SP5K2RHMSBH4PAP4PGX77MCVNK1ZEED07CWX9TJT.stakeflow-staking-mainnet-v2 release-nft token-id staker))
    ;; Update fees collected
    (var-set total-fees-collected (+ (var-get total-fees-collected) (var-get unstake-fee)))
    (ok true)
