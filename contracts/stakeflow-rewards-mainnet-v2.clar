...
(define-read-only (calculate-rewards (token-id uint))
  (let
    (
      ;; V2: Reference the v2 staking contract (uses full address)
      (stake-info (contract-call? 'SP5K2RHMSBH4PAP4PGX77MCVNK1ZEED07CWX9TJT.stakeflow-staking-mainnet-v2 get-stake-info token-id))
    )
    (match stake-info
      stake-data
        (let
...
(define-public (claim-rewards (token-id uint))
  (let
    ...
    ;; Mint rewards to staker (token contract stays the same)
    (try! (contract-call? .stakeflow-token-mainnet mint-for-staking rewards staker))
    ;; Update last claim (V2: Reference the v2 staking contract - uses full address)
    (try! (contract-call? 'SP5K2RHMSBH4PAP4PGX77MCVNK1ZEED07CWX9TJT.stakeflow-staking-mainnet-v2 update-last-claim token-id))
