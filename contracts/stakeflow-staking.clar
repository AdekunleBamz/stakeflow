...
(define-public (stake-nft (token-id uint))
  (let
    (
      (nft-owner (unwrap! (contract-call? .stakeflow-nft get-owner token-id) ERR-NOT-TOKEN-OWNER))
    )
    ;; Check not paused
    ...
    (try! (contract-call? .stakeflow-nft transfer token-id tx-sender (as-contract tx-sender)))
    ;; Record stake
    (map-set staked-nfts
      { token-id: token-id }
      {
...
(define-public (remove-stake (token-id uint))
  (let
    ...
    (try! (as-contract (contract-call? .stakeflow-nft transfer token-id tx-sender original-owner)))
    ;; Remove stake record
...
(define-public (update-last-claim (token-id uint))
  (let
    ...
    (asserts! (is-eq contract-caller .stakeflow-rewards) ERR-NOT-AUTHORIZED)
    (map-set staked-nfts
      { token-id: token-id }
