...
(define-read-only (calculate-rewards (token-id uint))
  (let
    (
      (stake-info (contract-call? .stakeflow-staking get-stake-info token-id))
    )
    (match stake-info
      stake-data
        (let
...
(define-public (claim-rewards (token-id uint))
  (let
    (
      (stake-info (unwrap! (contract-call? .stakeflow-staking get-stake-info token-id) ERR-NOT-STAKED))
    ...
    (try! (contract-call? .stakeflow-token mint-for-staking rewards staker))
    ;; Update last claim
    (try! (contract-call? .stakeflow-staking update-last-claim token-id))
...
(define-public (claim-and-distribute (token-id uint) (recipient principal))
  (let
    ...
    (asserts! (is-eq contract-caller .stakeflow-unstake) ERR-NOT-AUTHORIZED)
    ;; If there are rewards, mint them
    (if (> rewards u0)
      (begin
        (try! (contract-call? .stakeflow-token mint-for-staking rewards recipient))
        (var-set total-rewards-distributed (+ (var-get total-rewards-distributed) rewards))
        (ok rewards)
      )
      (ok u0)
